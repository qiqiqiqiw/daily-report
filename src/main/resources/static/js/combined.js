const CombinedAPI = {
    ...
    getMonthlyReports(month) {
        return this.request(`?month=${month}`);
    },
...
const App = {
    ...
    async loadCalendarData() {
        ...
        try {
            const reports = await CombinedAPI.getMonthlyReports(month);
    ...
    async onDateClick(dateStr, cellEl) {
        ...
        document.getElementById('modalDateTitle').textContent =
            `${d.getFullYear()}年${d.getMonth() + 1}月${d.getDate()}日 ${weekdays[d.getDay()]} 日报`;

        this.setReportState('loading');
        this.openModalEl('reportModal', 'reportModalContent', cellEl);

        try {
            const report = await CombinedAPI.getReportByDate(dateStr);
            if (report) {
    ...
    async generateReport() {
        this.setReportState('loading');
        try {
            const report = await CombinedAPI.generateReport(this.selectedDate);
            this.currentReport = report;
            this.setReportState('view');
